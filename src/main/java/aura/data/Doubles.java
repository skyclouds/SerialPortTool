...
import static aura.data.Preconditions.checkElementIndex;
import static aura.data.Preconditions.checkNotNull;
import static aura.data.Preconditions.checkPositionIndexes;
...
public final class Doubles {
     ...
     * returned is the same as that of
     * <code>((Double) a).{@linkplain Double#compareTo compareTo}(b)</code>. As with
     * that method, {@code NaN} is treated as greater than all other values, and
     * {@code 0.0 > -0.0}.
     * <p>
     * <p>
     ...
     * {@code array}. Note that this always returns {@code false} when
     ...
     * returned unchanged. If {@code value} is less than {@code min}, {@code min} is
     * returned, and if {@code
     * value} is greater than {@code max}, {@code max} is returned.
     *
     * @param value the {@code double} value to constrain
     * @param min   the lower bound (inclusive) of the range to constrain
     *              {@code value} to
     * @param max   the upper bound (inclusive) of the range to constrain
     *              {@code value} to
    ...
    /**
     * Returns a comparator that compares two {@code double} arrays <a href=
     * "http://en.wikipedia.org/wiki/Lexicographical_order">lexicographically</a>.
     * That is, it compares, using {@link #compare(double, double)}), the first pair
     * of values that follow any common prefix, or when one array is a prefix of the
     ...
     * <p>
     * <p>
     * Note that this method uses the total order imposed by {@link Double#compare},
     ...
     * Collections.reverse(Doubles.asList(array).subList(fromIndex, toIndex))}, but
     ...
     * <p>
     * <p>
     * The returned list may have unexpected behavior if it contains {@code NaN}, or
     * if {@code NaN} is used as a parameter to any of its methods.
     * <p>
     *
